...
class Agent(object):
    ...
    def __init__(
        ...
        assert utils.is_normalized(self.D), "D matrix is not normalized (i.e. D.sum(axis = 0) must all equal 1.0" # this used to say A matrix incorrectly it seems -Shawn
    ...
    # Limitation, only can specify up to state factor 3
    def set_qs(self, qs=None, state_factor='all'):
        if hasattr(self, "qs_hist"):
            if state_factor=='all':
                self.qs_hist.append(qs)
                new_qs=qs
            elif state_factor=='1':
                new_qs=self.qs_hist[-1]
                new_qs[0]=qs
                self.qs_hist.append(new_qs)
            elif state_factor=='2':
                new_qs=self.qs_hist[-1]
                new_qs[1]=qs
                self.qs_hist.append(new_qs)
